# Agent request — declaration-by-email: Declaration by Email
...
- **Options:** (a) a fixed structured template only · (b) free-text with field extraction only · (c) a structured template recommended, with free-text accepted and best-effort extraction.
