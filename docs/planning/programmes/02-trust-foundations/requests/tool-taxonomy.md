# Agent request — tool-taxonomy: Tool Taxonomy
...
- **Options:** (a) Reverse-domain prefix (e.g. `org.example.toolname`) · (b) `x-` prefix (e.g. `x-mytool`) · (c) Both, with reverse-domain for parties that own a domain and `x-` as a fallback.
...
- **Options:** (a) Keep exactly four types now, and subdivide later only when real contribution volume shows the need · (b) Add a `service` type now · (c) Add `service` and `dataset` types now.
