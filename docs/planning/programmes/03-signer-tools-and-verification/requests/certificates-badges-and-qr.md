# Agent request — certificates-badges-and-qr: Certificates, Badges, and QR
...
- **Options:** (a) No — the QR encodes only the bare status URL (single encoding) · (b) yes — a second encoding of the `declarationId` in a structured, documented format · (c) yes, but only as a short prefix/URI alongside the URL, not a full second record.
