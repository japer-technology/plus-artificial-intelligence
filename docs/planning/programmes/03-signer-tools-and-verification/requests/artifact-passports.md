# Agent request — artifact-passports: Artifact Passports
...
- **Options:** (a) On demand by the checker at check time (no storage; the snapshot is the check's own capture) · (b) stored by a passport service when the passport is published · (c) hybrid — self-hosted passports embed a snapshot; hosted passports may refresh a stored snapshot.
...
- **Options:** (a) Retain the last snapshot indefinitely with the passport (append-only; old snapshots stay readable) · (b) retain for 12 months after supersession, then archive · (c) retain only the current snapshot — supersession replaces it.
