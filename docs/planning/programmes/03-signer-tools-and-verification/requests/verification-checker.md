# Agent request — verification-checker: Verification Checker
...
- **Options:** (a) No cache — results are recomputed per request and returned directly, no shareable result link · (b) a short-lived cache (24 hours) with results marked by their original check date · (c) a 7-day cache for shareable links, always labelled with the original check date.
...
- **Options:** (a) 60 requests/hour per client and 10 fetches/minute per target domain · (b) 100 requests/hour per client and 30 fetches/minute per target domain · (c) 30 requests/minute per client and 1 fetch per 10 seconds per target domain.
