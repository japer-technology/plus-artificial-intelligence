# Agent request — signing-wizard: Signing Wizard
...
- **Options:** (a) Yes — a single recap screen listing every field about to be asserted, with edit links, immediately before the issue action · (b) No — the final step already presents the visible text; an extra screen adds friction to the shortest path · (c) Yes, but only on the optional (non-shortest) paths, never on the shortest path.
...
- **Options:** (a) 5 participants per language; pass = all complete the shortest path unassisted and none report a field as coercive · (b) 3 participants per language; pass = shortest path completes with no blocker · (c) 8 participants per language with a scripted success-rate threshold (90% or higher complete).
...
13. Self-check the result against §4 acceptance criteria before finishing.
