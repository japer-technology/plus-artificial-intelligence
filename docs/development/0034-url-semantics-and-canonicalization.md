# Development task — url-semantics-and-canonicalization: URL Semantics and Canonicalisation
...
- **Why now / risk of deferring:** It is a P1 member — before named subdomains and hosted profiles build URL products on top (mini-plan Order). Deferring lets every URL-bearing sibling mint addresses under no shared scheme, so canonical identity and never-repurpose rules would have to be retrofitted (spec §2).
- **Features to deliver:**
  - The path/query/fragment division: paths carry durable identity, queries carry presentation/filters, fragments carry local drafts; structured meaning lives in the linked manifest, not the URL.
  - Canonical-URL and immutable-ID rules; slug/alias/redirect rules; UTF-8 and `%2B` encoding; sorted parameters; bounded segments; tombstone resolution; a version-bearing vs unversioned-redirect scheme.
- **Depends on:** specification-versioning-and-hashing, declaration-lifecycle, continuity-and-namespace-custody
...
- **Option (a):** 2048 characters total, 255 characters per path segment, 1024 characters per query value
  - **For:** §5.2.3 requires URL length to be bounded to a documented maximum with per-segment and per-query-value bounds, and 2048/255/1024 are the widely compatible browser/server-safe defaults, so a conservative published number satisfies the requirement now.
  - **Against:** The numbers are this request's proposal, not spec-derived — §10 says the bounds are "to be set with compatibility testing", so a hard number is provisional.
- **Option (b):** 1024 characters total, 128 per segment, 512 per query value
  - **For:** More conservative bounds are safer for edge clients and proxies.
  - **Against:** Unnecessarily tight — may reject legitimate long query values — and equally ungrounded in the spec.
- **Option (c):** Adopt no fixed number yet — publish "bounded" with the bound deferred to compatibility testing.
  - **For:** Honest to §10's "to be set with compatibility testing" — deferring avoids committing to numbers before testing.
  - **Against:** §5.2.3 requires a documented bound now, so "bounded" with no number is an unbound placeholder that fails the requirement.
...
- **Option (a):** Keep slugs as a permanent human-friendly layer alongside the immutable ID (ID remains authoritative)
  - **For:** §5.2.1 says "a slug MAY appear alongside it for humans but the ID is authoritative", and §5.2.7 defines a documented slug scheme with collision disambiguation — slugs are already an optional human layer with no drift cost.
  - **Against:** Maintaining slug disambiguation and alias records is ongoing work.
- **Option (b):** Deprecate slugs now in favour of ID-only URLs
  - **For:** ID-only URLs are simpler and remove any ambiguity about authority.
  - **Against:** Contradicts §5.2.1/§5.2.7, which already design slugs as an optional human layer, and drops the human-friendly affordance with no drift benefit.
- **Option (c):** Keep slugs on hosted products but not on the canonical form.
  - **For:** Restricts slugs to hosted products where humans benefit most.
  - **Against:** §5.2.1 places the slug on the canonical path itself ("a slug MAY appear alongside it"), so the canonical form already accommodates slugs — (c) is neither simpler nor spec-aligned.
